...
viewModel.subscribeTheme((theme) => {
  if (theme === 'dark') {
...
viewModel.subscribeItems((items) => {
  let todoItems = '';
...
themeToggle.addEventListener('click', () => viewModel.toggleTheme());
todoForm.addEventListener('submit', (event) => {
  event.preventDefault();
  const text = todoInput.value.trim();
  if (text) {
    viewModel.addItem({ id: Date.now(), name: text });
    todoInput.value = '';
  }
});
